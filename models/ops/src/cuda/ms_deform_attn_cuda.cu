    ...
    const int im2col_step)
{
    ...
    AT_ASSERTM(value.is_cuda(), "value must be CUDA");
    AT_ASSERTM(spatial_shapes.is_cuda(), "spatial_shapes must be CUDA");
    AT_ASSERTM(level_start_index.is_cuda(), "level_start_index must be CUDA");
    AT_ASSERTM(sampling_loc.is_cuda(), "sampling_loc must be CUDA");
    AT_ASSERTM(attn_weight.is_cuda(), "attn_weight must be CUDA");
